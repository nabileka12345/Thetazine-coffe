...
async function fetchMenu() {
    ...
    try {
        // API Laravel untuk mendapatkan products
        ...
        // Transform data dari Laravel ke format yang dibutuhkan
        // Laravel returns: {data: {data: [...], ...}}
        let products = result.data.data || result.data || [];
        
        // Map product data ke format yang dibutuhkan untuk render
        return products.map(product => {
            // 1) Utamakan URL gambar dari database (image_url)
            let imagePath = (product.image_url && String(product.image_url).trim().length > 0)
                ? product.image_url
                : null;

            // 2) Jika tidak ada image_url, baru gunakan fallback lokal berdasarkan nama
            if (!imagePath) {
                const nameLower = (product.name || '').toLowerCase();
                if (nameLower.includes('cappuccino') || nameLower.includes('cappucino')) {
                    imagePath = 'img/cappucino.jpg';
                } else if (nameLower.includes('matcha') || nameLower.includes('latte')) {
                    imagePath = 'img/matchalatte.jpg';
                } else if (nameLower.includes('caramel') || nameLower.includes('macchiato')) {
                    imagePath = 'img/caramelmachiato.jpg';
                } else if (nameLower.includes('iced')) {
                    imagePath = 'img/icedlatte.jpg';
                    ...
                    imagePath = 'img/chocolatemocha.jpg';
                } else {
                    imagePath = 'img/cappucino.jpg'; // default ultimate fallback
                }
            }
            ...
            return {
                ...
                description: product.description || 'Nikmati cita rasa kopi terbaik kami'
            };
        });
                ...
                <div class="relative w-full h-full">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center p-4">
                        <div class="w-48 h-48 rounded-full overflow-hidden border-4 border-[#EFDBC4]/20 shadow-inner bg-white transition-transform duration-300 group-hover:scale-95">
                            <img src="${item.image}" alt="${item.name}" class="w-full h-full object-cover" onerror="this.src='img/cappucino.jpg'">
                        </div>
                    </div>
...
// Update cart badge and visibility
function updateCartUI() {
    ...
    // Show/hide cart button based on login and cart content
    const authToken = localStorage.getItem('auth_token');
